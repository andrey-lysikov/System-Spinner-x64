...
public sealed class FpsCounter : IDisposable
{
    ...
    // The graphics kernel event numbers are undocumented and differ between builds, so the first
    // second is spent listening: an event can arrive many times per frame, so the rate matters.
