...
internal static class AudioEndpoint
{
    ...
    // PreserveSig on purpose: without it a failing HRESULT becomes an exception and the returned
    // int comes from a slot the callee never wrote, so a sleeping monitor would throw.
    ...
    private static IAudioEndpointVolume? Open(out string name)
    {
        ...
        {
            // The volume interface stands on its own now. Released here rather than left to the
            // collector: a held volume key opens the endpoint dozens of times a second.
            Release(device);
            Release(enumerator);
        }
    }
    ...
    private static T With<T>(Func<IAudioEndpointVolume, T> use, T whenUnavailable)
    {
        ...
        {
            // The device answered when opened and stopped a moment later: the screen carrying the
            // sound slept, the dock was unplugged. Throwing would take the key hook down with it.
